...
def time_domain_process(
    ...
    # generate signal and window
    signal_analog = hps.generate_signal(A0=orign_freq_amp, freq=sensor_freq, nA0=noise_freq_amp, nfreq=noise_freq)
    osignal_quant, _ = hps.adc_quantize(signal_analog, V_REF, ADC_BITS)
    signal, window_val = hps.apply_window(osignal_quant, win_type)
    
    # 当相邻两个样本的乘积为负时，表示发生了过零点
    products = np.multiply(signal[:-1], signal[1:])
    zero_crossings = np.sum(products < 0)
    ...
    for idx in zero_indices:
        ...
        # 检查前一个和后一个非零值
        prev_nonzero_idx = idx - 1
        next_nonzero_idx = idx + 1
        
        # 找到前一个非零值
        while prev_nonzero_idx >= 0 and signal[prev_nonzero_idx] == 0:
            prev_nonzero_idx -= 1
        
        # 找到后一个非零值
        while next_nonzero_idx < len(signal) and signal[next_nonzero_idx] == 0:
            next_nonzero_idx += 1
        
        # 如果前后都有非零值且符号不同，则算一次过零点
        if prev_nonzero_idx >= 0 and next_nonzero_idx < len(signal):
            if signal[prev_nonzero_idx] * signal[next_nonzero_idx] < 0:
                zero_crossings += 1
    
    tdp_est_sensor_freq = zero_crossings / (hps.DT() * 2)
    
    return tdp_est_sensor_freq
...
if __name__ == "__main__":
    ...
    for fq in orign_freq:
        ...
        tdp_sensor_freq = time_domain_process(hps=hps, sensor_freq=fq, orign_freq_amp=orign_freq_amp, noise_freq=noise_freq, noise_freq_amp=noise_freq_amp, V_REF=V_REF, ADC_BITS=ADC_BITS)
        print(f"tdp_sensor_freq={tdp_sensor_freq:4.6f}Hz")
        print(f"sensor_freq={sensor_freq:4.6f}Hz")
        print(f"sensor_amplitude={sensor_amplitude:4.6f}")
